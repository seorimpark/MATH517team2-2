...
boxplot(stacked_df1$distribution ~ stacked_df1$landcover, main="Distribution of land covers",lwd = 0.4,alpha = 0.3, xlab="percentages", ylab="land covers",outline=FALSE)
#horizontal = TRUE, 
```
In Figure() is displayed the distribution of land covers using Boxplot. We can see that most of the land cover represent less than 10% of the the area considered. Meaning that the areas considered are very diverse. 
...
$$Wspeed_{new}=\sqrt{Wspeed_{East}^2 + Wspeed_{West}^2 }$$

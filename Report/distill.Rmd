...
1. How have the number of fires in the United States of America evolved with time since 1993?

2. How do the land covers vary over time? 

3. What are the main factors that cause wildfires ? 
...
A descriptive and visual approach where we will produce interactive plots with information on the different dimensions : geographically temporally and by different factors. 
Afterward we will look at the distribution of land covers. We will also analyse the variation of land covers in the same location since we noticed it changed in some areas. 
We will then proceed with the analysis of the number of fires. To do so, we will look at the land covers and meteorological parameters. We noticed that the correlation between parameters was low, we used subsets and we did a quantile regression. 

## Sources of information / datasets
To perform this analysis, we use a dataset for the United States from 1993 to 2015. It contains 563,983 rows with 37 columns. The columns are the following:
...
We can see from the summary that wildfires remain relatively rare events with more than 75% of locations have less than 2 fires per month considering the feature CNT. Same applies for the feature BA representing aggregated burnt area, where the distribution is strongly positively skewed. 
...
```
In Figure(), we can see the distribution of land covers using Boxplot. We can see that most of the land cover represent less than 10% of the the area considered. Meaning that the areas considered are very diverse. 
...
The given dataset had a list of different coordinates in the United States. To determine which coordinates belong to which state, we used the python library $\it{reverse\_geocoder}$. This library gives the closest address given the coordinates, so we took the name of the state out of it and added up the numbers and stored in a dictionary for each state. 

To visualize the data well, we had to make several adjustments. First, we divided the number of incidences and the burnt area by the total area of the states to make a comparison. Then we multiplied the value by 10⁶ so that we get the number of wildfires/burnt area of the state per 10⁶km². Also, we realized that the obtained numbers could go from the order of $10^{-2}$ to $10^{3}$, so we took the log of them to have a reasonable color scale for each state. The final numbers for the plots are calculated in the following way :

$$Final\_CNT_i=log_2\left(\frac{(\sum_{k=0}^{total\_number\_of\_coordinates\_in\_i}CNT_k)*10^6}{total\_area\_of\_i}+1\right)\; for\;i=\{list\;of\;US\;states\}$$
$$Final\_BA_i=log_2\left(\frac{(\sum_{k=0}^{total\_number\_of\_coordinates\_in\_i}BA_k)*10^6}{total\_area\_of\_i}+1\right)\; for\;i=\{list\;of\;US\;states\}$$
...
$$Local\_CNT_k=4log_2\left(CNT_k+1\right)\; for\;k=\{list\;of\;coordinates\}$$

$$Local\_BA_k=2log_2\left(BA_k+1\right)\; for\;k=\{list\;of\;coordinates\}$$

One main aspect that we can observe is that the number of incidences does not always match with the burnt area. For instance, on May 1993, Texas had about $10^3$ acres burnt per 10⁶km², but as shown on the Figure() , the number of incidences was not as huge compared to the burnt area seeing the number and the size of red dots in Texas. 

![Plot in 05/1993](1.png)
...
We will first proceed by representing the territory according to the predominant type of coverage. Figure () shows this arrangement in 2000. We observe that water dominates the maritime borders, tree broadleaved are predominant in the east of the country while the west is dominated by tree needleleave. We can easily identify the cities of New York, Los angeles and other large cities by their predominance of urban space.
...
To analyze the variation, we create a vector whose values start at zero for any given location and increases by one as soon as the predominant coverage area changes from one month  to the next. What is first interesting to note is that all locations change their predominant landcover at least once. Indeed in Figure (), we see that the shift values start at 1. We also see that the shifts do not exceed 5, knowing that this measurement is taken over 22 years, this is still relatively low. Note that we will not further research on the reason behind these changes during this project due to lack of time.
...
We remark that most of the values are close to 0 and because of the outliers we can't see the distribution very good. So we are going to filter the outliers to have a better view of the distrubution 
